...
local get_compiled_path = function(theme)
  return table.concat({ vim.fn.stdpath("state"), "galax", theme .. "_compiled.lua" }, PATH_SEP)
end
...
function M.compile(theme, highlights, termcolors)
  vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "galax", 448)

  local fname = get_compiled_path(theme)
  local file, err = io.open(fname, "wb")
  if not file or err then
    vim.notify("Galax: Error writing " .. fname .. ":\n" .. err, vim.log.levels.ERROR)
    return
  end

  local lines = {
    "require'galax'.compiled = string.dump(function()",
    "local g = vim.g",
  ...
  file:write(require("galax").compiled)
  file:close()
end
